    ...
    static isCommentRule(raw) {
        ...
        if (trimmed.startsWith(exports.CommentMarker.Hashmark)) {
            ...
            if (!trimmed[end + 1]
                ...
                || (trimmed[end + 1] === exports.CommentMarker.Hashmark && trimmed[end + 2] === exports.CommentMarker.Hashmark)) {
                return true;
            }
        }
...
class CssTree {
    ...
    static isExtendedCssNode(node, pseudoClasses, attributeSelectors) {
        return ((node.type === exports.CssTreeNodeType.PseudoClassSelector && pseudoClasses.has(node.name))
            || (node.type === exports.CssTreeNodeType.AttributeSelector && attributeSelectors.has(node.name.name)));
    }
    /**
    ...
    static parsePseudoClassArgumentAsNumber(node) {
        ...
        if (argument.type !== exports.CssTreeNodeType.String
            && argument.type !== exports.CssTreeNodeType.Number
            && argument.type !== exports.CssTreeNodeType.Raw) {
    ...
    static generatePseudoClassValue(node) {
        ...
        node.children?.forEach((child) => {
            switch (child.type) {
                case exports.CssTreeNodeType.Selector:
                ...
                default:
                    ...
                    result += ecssTree.generate(child);
            }
        });
        return result;
    }
    ...
    static parse(raw, loc = defaultLocation) {
        ...
        if (injection.type === exports.CssTreeNodeType.Atrule) {
            ...
            if (!atrule.prelude || atrule.prelude.type !== exports.CssTreeNodeType.AtrulePrelude) {
                throw new AdblockSyntaxError(
                // eslint-disable-next-line max-len
                `Invalid at-rule prelude, expected '${exports.CssTreeNodeType.AtrulePrelude}' but got '${atrule.prelude?.type ?? NONE}' instead`, {
                    start: atrule.loc?.start ?? loc,
                    end: atrule.loc?.end ?? shiftLoc(loc, raw.length),
                });
            }
            ...
            if (!atrule.block || atrule.block.type !== exports.CssTreeNodeType.Block) {
                throw new AdblockSyntaxError(
                // eslint-disable-next-line max-len
                `Invalid at-rule block, expected '${exports.CssTreeNodeType.Block}' but got '${atrule.block?.type ?? NONE}' instead`, {
                    start: atrule.loc?.start ?? loc,
                    end: atrule.loc?.end ?? shiftLoc(loc, raw.length),
                });
            }
...
function handleBoundingCombinators(ref, name) {
    ...
    if (ref.item.prev?.data.type === exports.CssTreeNodeType.Combinator) {
    ...
    if (ref.item.next?.data.type === exports.CssTreeNodeType.Combinator) {
        // Special case is space combinator, it's allowed, but should be removed
...
function extractUboModifiersFromSelector(selector) {
    ...
    ecssTree.walk(cleaned, {
        enter: (node, item, list) => {
            // Don't take into account selectors and selector lists
            if (node.type === exports.CssTreeNodeType.Selector || node.type === exports.CssTreeNodeType.SelectorList) {
                return;
            }
            // Set the :not() reference if we are on the top level
            if (node.type === exports.CssTreeNodeType.PseudoClassSelector && node.name === NOT_OPERATOR && depth === 0) {
                notRef = {
                    node,
                    item,
                    list,
    ...
    return {
        modifiers,
        cleaned: ecssTree.toPlainObject(cleaned),
    };
}
/**
...
function extractUboModifiersFromSelectorList(selectorList) {
    ...
    cleaned.children.forEach((child) => {
        if (child.type === exports.CssTreeNodeType.Selector) {
...
class CosmeticRuleParser {
    ...
    static parse(raw, loc = defaultLocation) {
        ...
        switch (separatorResult.separator) {
            ...
            case '#@?#':
                ...
                if (CssInjectionBodyParser.isUboCssInjection(rawBody)) {
                    ...
                    if (hasUboModifierIndicator(rawBody)) {
                        ...
                        if (extractedUboModifiers.modifiers.children.length > 0) {
                            ...
                            uboCssInjectionRuleNode.syntax = exports.AdblockSyntax.Ubo;
                        }
                    }
                ...
                if (hasUboModifierIndicator(rawBody)) {
                    ...
                    if (extractedUboModifiers.modifiers.children.length > 0) {
                        ...
                        elementHidingRuleNode.syntax = exports.AdblockSyntax.Ubo;
                    }
                }
            ...
            case '#@#^':
                ...
                if (hasUboModifierIndicator(rawBody)
                    && uboHtmlRuleNode.body.body.type === exports.CssTreeNodeType.SelectorList) {
                    // eslint-disable-next-line max-len
                    ...
                    if (extractedUboModifiers.modifiers.children.length > 0) {
                        ...
                        uboHtmlRuleNode.syntax = exports.AdblockSyntax.Ubo;
                    }
                }
    ...
    static generate(ast) {
        ...
        // uBO rule modifiers
        if (ast.syntax === exports.AdblockSyntax.Ubo && ast.modifiers) {
...
class ModifierValidator {
    ...
    getAdgDocumentationLink = (modifier) => {
        ...
        return getBlockerDocumentationLink(this.modifiersData, BLOCKER_PREFIX[exports.AdblockSyntax.Adg], modifier);
    };
    /**
    ...
    getUboDocumentationLink = (modifier) => {
        ...
        return getBlockerDocumentationLink(this.modifiersData, BLOCKER_PREFIX[exports.AdblockSyntax.Ubo], modifier);
    };
    /**
    ...
    getAbpDocumentationLink = (modifier) => {
        ...
        return getBlockerDocumentationLink(this.modifiersData, BLOCKER_PREFIX[exports.AdblockSyntax.Abp], modifier);
    };
}
            ...
            for (let i = 0; i < selector.children.length; i += 1) {
                ...
                switch (node.type) {
                    ...
                    case exports.CssTreeNodeType.PseudoClassSelector:
                        ...
                        if (arg.type !== exports.CssTreeNodeType.String
                            && arg.type !== exports.CssTreeNodeType.Raw
                            && arg.type !== exports.CssTreeNodeType.Number) {
        ...
        leave: (node) => {
            ...
            if (node.type === exports.CssTreeNodeType.AttributeSelector
                ...
                if ([PseudoClasses.Has, PseudoClasses.Not].includes(name)) {
                    ...
                    if (value.type === exports.CssTreeNodeType.Identifier) {
                        children.push(value);
                    }
                    ...
                    children.push({
                        type: exports.CssTreeNodeType.Raw,
                        value,
                    });
                }
                ...
                // selector
                const pseudoNode = {
                    type: exports.CssTreeNodeType.PseudoClassSelector,
                    name,
                    children,
                };
...
exports.FilterListConverter = FilterListConverter;
