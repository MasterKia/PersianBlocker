...
import { version, LinterCli, LinterConsoleReporter } from './aglint.esm.js';
 ...
 */
const getConfigFileContent = (chosenFormat, chosenSyntaxes) => {
    ...
    const preparedConfig = {
        ...
        syntax: chosenSyntaxes.length === 0 ? ["Common" /* AdblockSyntax.Common */] : chosenSyntaxes,
    };
...
const createConfig = async (cwd) => {
    ...
    const chosenSyntaxes = await checkbox({
        ...
        choices: [
            { value: "AdblockPlus" /* AdblockSyntax.Abp */ },
            { value: "AdGuard" /* AdblockSyntax.Adg */ },
            { value: "UblockOrigin" /* AdblockSyntax.Ubo */ },
        ],
    });
...
(async () => {
    try {
        ...
        program
            ...
            // Options
            .option('-f, --fix', 'Enable automatic fix, if possible (BE CAREFUL, this overwrites original files with the fixed ones)', false)
        ...
        const cli = new LinterCli(new LinterConsoleReporter(program.opts().colors), !!program.opts().fix, !!program.opts().ignores);
