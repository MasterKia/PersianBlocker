...
function severity() {
  ...
  // @ts-ignore 2322
  return superstruct.define('severity', value => {
    if (typeof value === 'string') {
      return SEVERITY_NAMES.includes(value) || `Expected a valid severity string (${SEVERITY_NAMES.join(', ')}), but received ${value}`;
    }
    if (typeof value === 'number') {
 ...
 */
const linterRuleConfigSchema = superstruct.union([severity(), configArray()]);
 ...
 */
const linterRulesSchema = superstruct.optional(superstruct.record(superstruct.string(), linterRuleConfigSchema));
/**
...
const linterConfigPropsSchema = {
  root: superstruct.optional(superstruct.boolean()),
  extends: superstruct.optional(superstruct.array(superstruct.string())),
  allowInlineConfig: superstruct.optional(superstruct.boolean()),
  syntax: superstruct.optional(superstruct.array(superstruct.enums(["Common" /* AdblockSyntax.Common */, "AdGuard" /* AdblockSyntax.Adg */, "UblockOrigin" /* AdblockSyntax.Ubo */, "AdblockPlus" /* AdblockSyntax.Abp */]))),
 ...
 * Superstruct schema for the linter rule config (used for validation)
 */
const linterConfigSchema = superstruct.object(linterConfigPropsSchema);
/**
    ...
    onRule: context => {
      ...
      if (ast.category === "Comment" /* RuleCategory.Comment */ && ast.type === "HintCommentRule" /* CommentRuleType.HintCommentRule */) {
        ...
        for (const hint of ast.children) {
          ...
          if (name !== PLATFORM$2 && name !== NOT_PLATFORM$2 || !hint.params || hint.params.children.length < 2) {
            continue;
          }
...
const KNOWN_HINTS = new Set([NOT_OPTIMIZED, PLATFORM$1, NOT_PLATFORM$1]);
// https://adguard.com/kb/general/ad-filtering/create-own-filters/#platform-and-not_platform-hints
const KNOWN_PLATFORMS = new Set(['windows', 'mac', 'android', 'ios', 'ext_chromium', 'ext_ff', 'ext_edge', 'ext_opera', 'ext_safari', 'ext_android_cb', 'ext_ublock']);
/**
...
const SUPPORTED_PREPROCESSOR_DIRECTIVES = new Set([ELSE_DIRECTIVE, ENDIF_DIRECTIVE, IF_DIRECTIVE, INCLUDE_DIRECTIVE, SAFARI_CB_AFFINITY_DIRECTIVE]);
...
const IfClosed = {
  ...
  events: {
    ...
    onRule: context => {
      ...
      if (rule.category !== "Comment" /* RuleCategory.Comment */ || rule.type !== "PreProcessorCommentRule" /* CommentRuleType.PreProcessorCommentRule */) {
        return;
      }
    ...
    onRule: context => {
      ...
      if (ast.category === "Cosmetic" /* RuleCategory.Cosmetic */) {
        ...
        for (const node of ast.domains.children) {
          ...
          if (!agtree.DomainUtils.isValidDomainOrHostname(node.value)) {
            context.report({
      ...
      if (ast.category === "Network" /* RuleCategory.Network */) {
        ...
        validateSyntax.forEach(agent => {
          ...
          ast.modifiers.children.forEach(modifier => {
            const validationResult = agtree.modifierValidator.validate(agent, modifier, ast.exception);
  ...
  events: {
    onRule: context => {
      ...
      if (ast.category === "Comment" /* RuleCategory.Comment */ && ast.type === "HintCommentRule" /* CommentRuleType.HintCommentRule */) {
        ...
        for (const hint of ast.children) {
          for (const param of ((_hint$params = hint.params) === null || _hint$params === void 0 ? void 0 : _hint$params.children) ?? []) {
            var _hint$params;
...
const NoShortRules = {
  ...
  events: {
    onRule: context => {
      ...
      (ruleNode.category === "Cosmetic" /* RuleCategory.Cosmetic */ || ruleNode.category === "Network" /* RuleCategory.Network */) && ruleTextTrimmed.length < minRuleLength) {
        context.report({
...
const defaultLinterRules = new Map([['if-closed', IfClosed], ['single-selector', SingleSelector], ['duplicated-modifiers', DuplicatedModifiers], ['unknown-preprocessor-directives', UnknownPreProcessorDirectives], ['duplicated-hint-platforms', DuplicatedHintPlatforms], ['duplicated-hints', DuplicatedHints], ['unknown-hints-and-platforms', UnknownHintsAndPlatforms], ['invalid-domain-list', InvalidDomainList], ['invalid-modifiers', InvalidModifiers], ['inconsistent-hint-platforms', InconsistentHintPlatforms], ['no-short-rules', NoShortRules]]);
...
function validateLinterConfig(config) {
  try {
    ...
    superstruct.assert(config, linterConfigSchema);
  } catch (error) {
...
const defaultConfigPresets = new Map([['aglint:recommended', aglintRecommended], ['aglint:all', aglintAll]]);
...
class Linter {
  ...
  setRuleConfig(ruleName, ruleConfig) {
    ...
    if (config !== undefined) {
      ...
      try {
        superstruct.assert(config, entry.rule.meta.config.schema);
      } catch (err) {
  ...
  addRuleEx(name, data) {
    ...
    if (clone.configOverride) {
      if (!clone.rule.meta.config) {
        ...
        try {
          superstruct.assert(clone.configOverride, clone.rule.meta.config.schema);
        } catch (err) {
  ...
  getRuleConfig(name) {
    ...
    return [isSeverity(entry.severityOverride) ? entry.severityOverride : entry.rule.meta.severity, entry.configOverride || entry.rule.meta.config.default];
  }
  /**
  ...
  getRule(name) {
    var _this$rules$get;
    return cloneDeep((_this$rules$get = this.rules.get(name)) === null || _this$rules$get === void 0 ? void 0 : _this$rules$get.rule);
  }
  /**
  ...
  lint(content, fix = false) {
    ...
    const invokeEvent = event => {
      for (const [name, data] of this.rules) {
        ...
        if (data.rule.meta.config) {
          superstruct.assert(data.configOverride || data.rule.meta.config.default, data.rule.meta.config.schema);
        }
        const genericContext = Object.freeze({
          ...
          // Rule configuration
          config: data.configOverride || ((_data$rule$meta$confi = data.rule.meta.config) === null || _data$rule$meta$confi === void 0 ? void 0 : _data$rule$meta$confi.default),
          // Reporter function
    ...
    const filterList = agtree.FilterListParser.parse(content);
      ...
      const code = (() => {
        ...
        if (ast.category === "Invalid" /* RuleCategory.Invalid */) {
              ...
              case ConfigCommentType.Main:
                {
                  if (ast.params && ast.params.type === 'Value') {
                    superstruct.assert(ast.params.value, linterRulesSchema);
          ...
          if (ast.category === "Comment" /* RuleCategory.Comment */ && ast.type === "AgentCommentRule" /* CommentRuleType.AgentCommentRule */) {
            ...
            this.config.syntax = agents.length > 0 ? agents : ["Common" /* AdblockSyntax.Common */];
          }
        }
    ...
    if (fix) {
      ...
      result.fixed = agtree.FilterListParser.generate(fixedFilterList, true);
    }
...
// aglint.config stuff
JSON_CONFIG_FILE_NAME, YAML_CONFIG_FILE_NAME, YML_CONFIG_FILE_NAME,
// .aglintrc stuff
RC_CONFIG_FILE, JSON_RC_CONFIG_FILE_NAME, YAML_RC_CONFIG_FILE_NAME, YML_RC_CONFIG_FILE_NAME]);
/**
 ...
 */
const SUPPORTED_EXTENSIONS = new Set(['.txt', '.adblock', '.adguard', '.ublock']);
/**
 * Problematic paths that should be ignored by default. This is essential to provide a good
 * experience for the user.
 */
const PROBLEMATIC_PATHS = ['node_modules', '.git', '.hg', '.svn', '.DS_Store', 'Thumbs.db'];
...
async function scan(dir, ignores = [], useIgnoreFiles = true) {
  ...
  if (useIgnoreFiles && items.includes(IGNORE_FILE_NAME)) {
    const content = await promises.readFile(path.join(dir, IGNORE_FILE_NAME), 'utf8');
...
async function configFinder(cwd, callback) {
  ...
  do {
    ...
    if (configFiles.length === 1) {
      ...
      if ((await callback(path.join(dir, configFiles[0]))) === true) {
        return;
      }
    ...
    dir = path.join(dir, UPPER_DIR);
  } while (dir !== path.resolve(dir, UPPER_DIR));
}
/**
...
class LinterCli {
  ...
  lintFile = async (file, config) => {
    ...
    // Lint the file
    let result = linter.lint(await promises.readFile(filePath, 'utf8'), this.fix || false);
    ...
    if (this.fix) {
      if (result.fixed) {
        await promises.writeFile(filePath, result.fixed);
...
class LinterConsoleReporter {
  ...
  onLintEnd = () => {
    ...
    output += SPACE;
    output += inflection.inflect('problem', total);
    output += SPACE + OPEN_PARENTHESIS;
    // Warnings
    output += this.chalk.yellow(`${this.warnings} ${inflection.inflect('warning', this.warnings)}`);
    output += COMMA + SPACE;
    // Errors
    output += this.chalk.red(`${this.errors} ${inflection.inflect('error', this.errors)}`);
    output += SPACE;
    output += 'and';
    output += SPACE;
    // Fatal errors
    output += this.chalk.red(`${this.fatals} fatal ${inflection.inflect('error', this.fatals)}`);
...
exports.LinterCli = LinterCli;
exports.LinterConsoleReporter = LinterConsoleReporter;
exports.NoConfigError = NoConfigError;
exports.buildConfigForDirectory = buildConfigForDirectory;
exports.configFinder = configFinder;
exports.findNextConfig = findNextConfig;
exports.findNextRootConfig = findNextRootConfig;
